...
lazy state should only be used when the useEffect is computationally expensive
lazy state only gets called when the state is defined in useState
this: React.useState(someExpensiveComputation()) To this: React.useState(() => someExpensiveComputation())

  function getInitialNameValue(params) {
